...
# The full list of units shown in BOTH dropdown selectors, in display order.
# The first nine are energy units; the last three are currencies.
UNITS = [
    "KWh", "MWh", "GWh", "TWh", "m3", "bcm", "th", "MMBtu", "tonnes",
    "EUR", "USD", "GBP",
]

# Which of the above are currencies (handled differently from energy units).
CURRENCIES = {"EUR", "USD", "GBP"}

# Energy conversion factors, ALL expressed relative to MWh (megawatt-hour),
# our chosen "base" unit. The value answers: "how many MWh is 1 of this unit?"
#
# To convert between two energy units we do:
#     result = amount * (factor[from_unit] / factor[to_unit])
#
# Example: 1 MMBtu = 0.29307 MWh, and 1 MWh = 1 MWh, so
#     500 MMBtu -> MWh  =  500 * (0.29307 / 1)  =  146.535 MWh
ENERGY_FACTORS = {
    "MWh": 1.0,            # base unit, by definition
    "KWh": 0.001,         # 1 KWh   = 0.001 MWh
    "GWh": 1_000.0,       # 1 GWh   = 1,000 MWh
    "TWh": 1_000_000.0,   # 1 TWh   = 1,000,000 MWh
    "MMBtu": 0.29307,     # 1 MMBtu = 0.29307 MWh
    "th": 0.029307,       # 1 therm = 0.029307 MWh
    "m3": 0.010556,       # 1 m3 of natural gas = 0.010556 MWh (standard CV)
    "bcm": 10_556_000.0,  # 1 bcm (billion cubic metres) = 10,556,000 MWh
    # NOTE: The tonne (of LNG) figure below is an APPROXIMATION. The real
    # energy content of a tonne of LNG depends on the calorific value of the
    # specific cargo, which varies. For production use you should source a more
    # precise, cargo-specific value. We use the same figure for both LNG and
    # Gas products for now to keep this first version simple.
    "tonnes": 1.4286,     # 1 tonne of LNG ~= 1.4286 MWh (approximate)
}

# Options offered by the optional "Per time period" quotient selector.
# IMPORTANT: the time period is purely COSMETIC. It changes the label printed
# on the result (e.g. "MWh/day") but does NOT change the numeric conversion.
...
class EnergyConverterApp(tk.Tk):
    ...
    def show_stage2(self):
        ...
        self.amount_entry = ttk.Entry(
            self.container, textvariable=self.amount_var,
            font=("Helvetica", 13), justify="center", width=20,
        )
        self.amount_entry.pack(pady=(5, 20))
    ...
    def _update_rate_line(self):
        ...
        # Work out how many "to" units equal 1 "from" unit.
        rate = self._unit_rate(from_u, to_u)
        # Format the number nicely (trim trailing zeros, sensible precision).
        self.rate_label.config(
            text=f"1 {from_u} = {self._fmt(rate)} {to_u}",
            style="Rate.TLabel",
        )
    ...
    def _unit_rate(self, from_u, to_u):
        ...
        if from_u in CURRENCIES:
            ...
            # Energy: factors are "MWh per unit". So 1 from-unit in to-units is:
            #   factor[from] / factor[to]
            return ENERGY_FACTORS[from_u] / ENERGY_FACTORS[to_u]
    ...
    def do_conversion(self):
        ...
        # Do the actual maths: amount * (rate of 1 from-unit in to-units).
        result = amount * self._unit_rate(from_u, to_u)

        # Build the time-period suffixes (cosmetic only). If the checkbox is
        # off, both suffixes are empty strings.
        from_suffix = ""
        to_suffix = ""
        if self.use_quotient.get():
        ...
        # e.g. "500 MMBtu/day = 146.5 MWh/day"
        self.result_label.config(
    ...
    def _fmt(number):
        ...
        - Small numbers (< 1) keep up to 8 decimals so tiny factors stay visible.
        ...
        if magnitude >= 1000:
            ...
            text = f"{number:.8f}"       # e.g. 0.00100000
